(() => {
  ...
  const copy = {
    ja: { required:'この項目は必須です', email:'正しいメールアドレスを入力してください', mismatch:'メールアドレスが一致しません', phone:'正しい電話番号を入力してください', sending:'送信中…', submit:'送信する', failure:'送信できませんでした。しばらくしてからもう一度お試しいただくか、tokumasullc@gmail.com まで直接メールをお送りください。' },
    en: { required:'This field is required.', email:'Please enter a valid email address.', mismatch:'Email addresses do not match.', phone:'Please enter a valid phone number.', sending:'Sending…', submit:'Send', failure:'Your message could not be sent. Please try again in a moment, or email us directly at tokumasullc@gmail.com.' }
  }[lang];
  ...
  const fields = ['name','inquiry','email','email_confirm','phone','country','postal_code','prefecture','address1','consent'];
  ...
  function payload() {
    ...
    ['name','inquiry','email','phone','country','postal_code','prefecture','address1','address2','address3','lang','_template','_honey'].forEach((name) => data.append(name,form.elements[name].value));
